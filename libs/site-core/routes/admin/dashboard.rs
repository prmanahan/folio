...
async fn get_dashboard(State(state): State<DbState>) -> Result<Json<DashboardCounts>, AppError> {
    ...
    let faq_responses: i64 =
        conn.query_row("SELECT COUNT(*) FROM faq_responses", [], |row| row.get(0))?;
    let gaps_weaknesses: i64 =
        conn.query_row("SELECT COUNT(*) FROM gaps_weaknesses", [], |row| row.get(0))?;
    let ai_instructions: i64 =
        conn.query_row("SELECT COUNT(*) FROM ai_instructions", [], |row| row.get(0))?;
