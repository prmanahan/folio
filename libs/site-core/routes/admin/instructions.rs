...
async fn get_instruction(
    ...
    let item = instructions::get_by_id(&conn, id).map_err(|e| match e {
        rusqlite::Error::QueryReturnedNoRows => AppError::NotFound(format!("Instruction {} not found", id)),
...
pub fn routes() -> Router<DbState> {
    Router::new()
        .route("/api/admin/instructions", get(list_instructions).post(create_instruction))
        .route(
            "/api/admin/instructions/{id}",
            get(get_instruction).put(update_instruction).delete(delete_instruction),
        )
}
...
mod tests {
    ...
    fn test_crud_cycle() {
        ...
        let updated = instructions::update(&conn, a.id, &make_input("Always be honest and direct", 10)).unwrap();
