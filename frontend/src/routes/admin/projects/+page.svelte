<script lang="ts">
  ...
  import { listProjects, createProject, updateProject, deleteProject } from '$lib/admin-api';
  import type { ProjectFull, ProjectInput } from '$lib/admin-types';
  import MarkdownEditor from '$lib/components/admin/MarkdownEditor.svelte';
  import TagInput from '$lib/components/admin/TagInput.svelte';
  import Toast from '$lib/components/admin/Toast.svelte';

  let items = $state<ProjectFull[]>([]);
  let mode = $state<'list' | 'edit'>('list');
  let editingId = $state<number | null>(null);
  let form = $state<ProjectInput>(emptyForm());
  ...
  onMount(async () => {
    try {
      ...
      toastMessage = err instanceof Error ? err.message : 'Failed to load projects';
      toastType = 'error';
    } finally {
      loading = false;
    }
  ...
  function startEdit(item: ProjectFull) {
    editingId = item.id;
    form = {
      title: item.title,
      slug: item.slug,
  ...
  async function handleSave() {
    ...
    try {
      ...
      mode = 'list';
      isDirty = false;
      toastMessage = editingId ? 'Project updated' : 'Project created';
      toastType = 'success';
    } catch (err) {
...
<div style="padding: 1.5rem 2rem 3rem; max-width: 56.25rem;">
  ...
  {:else if mode === 'list'}
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem;">
      <h1 style="font-size: 1.125rem; font-weight: 500; color: var(--nb-text); letter-spacing: -0.01em;">Projects</h1>
      <button
  ...
  {:else}
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.25rem;">
      <h1 style="font-size: 1.125rem; font-weight: 500; color: var(--nb-text); letter-spacing: -0.01em;">{editingId ? 'Edit Project' : 'New Project'}</h1>
    </div>
    ...
    <div style="background: var(--nb-bg2); border: 1px solid var(--nb-border); border-radius: 0.625rem; padding: 1.25rem; margin-bottom: 1rem;" oninput={() => isDirty = true}>
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.875rem;">
        ...
        <div style="grid-column: 1 / -1;">
          <div class="nb-label">Description</div>
          <MarkdownEditor bind:value={form.description} />
        </div>
        <div style="grid-column: 1 / -1;">
          <div class="nb-label">Tech Stack</div>
          <TagInput bind:value={form.tech_stack} />
        </div>
        <div style="grid-column: 1 / -1;">
        ...
        <div style="display: flex; align-items: center; gap: 0.5rem; padding-top: 1.25rem;">
          <input type="checkbox" id="field-published" bind:checked={form.published} onchange={() => isDirty = true} />
